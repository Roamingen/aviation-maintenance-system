...
*   **去中心化存储**：所有检修记录的核心数据（如注册号、工作描述、签名、航材消耗等）均存储在区块链上。
...
*   **多维度查询**：
    ...
    *   **按机械师**：查询特定机械师的工作记录。
*   **账户权限管理**：基于白名单的权限控制机制，只有获得授权的钱包地址才能参与记录签名和上链，确保数据来源的可靠性。
...
脚本运行完成后，你可以在前端页面通过搜索注册号（如 `B-1234`, `B-5678`）来查看这些记录。

---

## 🦊 MetaMask 使用指南 (可选)

虽然本系统默认使用后端托管钱包，但开发人员可以使用 MetaMask 连接到本地 Hardhat 网络进行调试或查看交易。

### 1. 配置网络
1.  安装 MetaMask 浏览器插件。
2.  打开 MetaMask -> 设置 -> 网络 -> 添加网络 -> 手动添加网络。
3.  填写以下信息：
...
### 2. 导入测试账户
Hardhat 启动时会生成 20 个预充值账户（每个 10000 ETH）。
1.  查看 `start_dev.ps1` 或 `npx hardhat node` 的终端输出，找到 "Account #0" 或其他账户的 **Private Key**。
2.  在 MetaMask 中点击顶部账户图标 -> **导入账户**。
3.  粘贴私钥并导入。

现在你可以看到账户余额，并可以在 MetaMask 中查看与合约的交互记录。

### 3. 使用自己的钱包 (授权与充值)

为了保证数据的安全性，本系统采用了**白名单授权机制**。只有被授权的钱包地址才能进行签名（互检、必检、放行）操作。未授权的钱包只能浏览数据。

如果你使用自己创建的 MetaMask 账户，请按照以下步骤获取权限：

**方法 A: 使用前端界面 (推荐)**

1.  启动系统并访问前端页面。
2.  在左侧导航栏点击 **"账户管理"** (Account Manager)。
3.  **授权钱包**: 在 "授权钱包" 卡片中输入你的 MetaMask 地址，点击授权。
4.  **获取测试币**: 在 "发送测试币" 卡片中输入你的地址，点击发送（用于支付 Gas 费）。
...
cd backend
# 授权
node authorize_wallet.js <你的MetaMask钱包地址>
# 充值
node fund_wallet.js <你的MetaMask钱包地址>
```
...
**Q: 提交记录时报错？**
A: 请检查后端日志。如果是**添加记录**，系统使用后端托管钱包自动签名。如果是**互检/放行签名**，请确保你的 MetaMask 钱包已连接且**已获得授权**（参考“账户管理”章节）。
